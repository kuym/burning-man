...
int main(void)
{
	...
	*/
	...
	enum StreamState
	{
		...
		StreamState_ActiveA0,		// => A1
		StreamState_ActiveA1,		// => A0 : Uninitialized
	};

	enum
	{
		Command_FrameCountBegin =	0,
		Command_Sync =				1,
		Command_IdentifyEndpoint =	2,
		Command_SetAttenuation =	3,
		Command_SetLowPassCorner =	4,
		Command_SetHighPassCorner =	5,
		Command_SetVisualEffects =	6,
	};

	unsigned int frameCount = 0, expectedCount = 0;
	while(true)
	{
		...
		while(gUARTFill > 0)
		{
			...
			case StreamState_ActiveA0:
				state = StreamState_ActiveA1;
				break;

			case StreamState_ActiveA1:
				state = (++frameCount < expectedCount)? StreamState_ActiveA0 : StreamState_Uninitialized;
				break;
			}
		}
