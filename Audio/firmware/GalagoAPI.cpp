////////////////////////////////////////////////////////////////
//
// ### THIS IS THE CANDIDATE FOR THE CANONICAL GalagoAPI.cpp ###
//
////////////////////////////////////////////////////////////////
...
void*			memcpy(void* dest, void const* source, size_t length)
{
	...
	unsigned char const* sb = (unsigned char const*)source;
	while(length && ((((size_t)db) | ((size_t)sb)) & 3))
	{
		*db++ = *sb++;
		length--;
	}
	
	if(((((size_t)dest) | ((size_t)source)) & 3) == 0)
	{
		unsigned int* di = (unsigned int*)db;
		unsigned int const* si = (unsigned int const*)sb;
		for(; length > 3; length -= 4)
			*di++ = *si++;
		db = (unsigned char*)di;
		sb = (unsigned char const*)si;
	}
...
int		NumberFormatter::format(byte* output, unsigned int number, int fractionBits, Base base)
{
	(void)fractionBits;
...
ssize_t					Buffer::indexOf(byte b, size_t offset)
{
	...
	for(size_t i = offset; i < _b->length; i++)
		if(_b->data[i] == b)
			return(i);
	return(-1);
}
	...
	*PowerDownControl &= ~PowerDownControl_SystemOscillator;
	...
	if(desiredFrequency > 12000000UL)	//must use PLL > 12MHz	@@to MAX(12e6, HARDWARE_EXTERNAL_CRYSTAL_FREQUENCY)
	{
		...
		*PLLSource = PLLSource_ExternalClock;
		...
		unsigned int bestM = 2, bestD = 1, smallestError = ~0, busFrequency;
		for(unsigned int m = 2; m < 32; m++)
		{
			...
			if(div == 3)						div = 2;
			else if((div == 4) || (div == 5))	div = 4;
			else if((div == 6) || (div == 7))	div = 8;
			else if(div > 8) div = 8;
			
			if(((HARDWARE_EXTERNAL_CRYSTAL_FREQUENCY * m) / div) > 72000000UL)	// may not exceed 72MHz
				continue;	// invalid, ignore this possibility
			
			int error = (int)desiredFrequency - (int)(busFrequency / div);
			if(error < 0)	error = -error;	// use abs(error)
			
			if(((unsigned int)error) < smallestError)
			{
				smallestError = error;
				bestM = m;
				bestD = div;
		...
		switch(bestD)
		{
		case 1:	bestD = 0; break;
		case 2:	bestD = 1; break;
		case 4:	bestD = 2; break;
		case 8:	bestD = 3; break;
		}
		
		//set the PLL's multiplication factor
		*PLLControl = (bestD << 5) | (bestM - 1);
...
bool    		System::overrideInterrupt(InterruptOverride interrupt, void (*handler)(void))
{
	switch(interrupt)
	{
	case System::UART:
		//ensure work queue is empty
		IOCore.irqUART = (handler != 0)? handler : &IO_onUARTInterrupt;
		return(true);
	case System::SPI:
		return(false);
	default:
...
void				IO_I2C_completePacket(IOCore::I2CTask* currentTask, bool success)
{
	...
	{
		unsigned int l = currentTask->len - 1;
